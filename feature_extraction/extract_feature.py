...
def extract_features(db_file, out_csv, max_rank=0):
    ...
    for row in c.execute(query):
        ...
        if (CHECK_ADBLOCK_STATUS_OF_ALL_JS or feat in SENSOR_FEATURES) and \
            ...
            # TODO: Buggy json disconnect file -> comment for now
...
def write_site_domain_counts(sensor_visit_ids,
    ...
    with open(site_counts_out, "w") as f:
        for sensor_feat, visit_ids in sensor_visit_ids.items():
    ...
    with open(domain_counts_out, "w") as f:
        for sensor_feat, domain_visit_dict in \
                sensor_script_domain_visit_ids.items():
            feat_name = sensor_feat.split("_")[-1]
            print("Total domains for feat %s: %d" % (feat_name,
                                               len(domain_visit_dict)))
            for script_domain, visit_ids in domain_visit_dict.items():
    ...
    with open(script_url_counts_out, "w") as f:
        for sensor_feat, script_url_visit_dict in \
                sensor_script_url_visit_ids.items():
            feat_name = sensor_feat.split("_")[-1]
            print("Total sites for feat %s: %d" % (feat_name,
                                             len(script_url_visit_dict)))
            for script_url, visit_ids in script_url_visit_dict.items():
                visit_ids_str = ",".join(str(visit_id) for visit_id in visit_ids)
    ...
    for script_address, visit_ids in canvas_reads.items():
        ...
        for canvas_rw_visit in canvas_rw_visits:
            ...
            if canvas_rw_visit in canvas_banned_calls[script_address]:
                print("Excluding potential canvas FP script", script_address,"visit#", canvas_rw_visit,
...
def get_adblock_rules():
    raw_easylist_rules = read_ab_rules_from_file('feature_extraction/adblock/easylist.txt')
    raw_easyprivacy_rules = read_ab_rules_from_file("feature_extraction/adblock/easyprivacy.txt")
    if ENABLE_UBLOCK:
        raw_ublock_rules = read_ab_rules_from_file("feature_extraction/adblock/adblock_blacklist_white.txt")
    else:
        raw_ublock_rules = []
...
if __name__ == '__main__':
    ...
    db = "feature_extraction/crawl-data.sqlite"
