...
DISCONNECT_JSON = "feature_extraction/adblock/disconnect.json"
...
def get_ps1_or_host(url):
    ...
    try:
        ...
        hostname = ps.urlparse(url).hostname
        try:
...
# TODO: BUGGY disconnect json file
def get_disconnect_blocked_hosts(disconnect_json=DISCONNECT_JSON):
    blocked_hosts = set()
    disconnect = json.dumps(open(disconnect_json).read())
...
def is_blocked_by_disconnect_old(url, disconnect_blocked_hosts):
    return ps.urlparse(url).hostname in disconnect_blocked_hosts


def is_blocked_by_disconnect(url, disconnect_blocked_hosts):
    host = ps.urlparse(url).hostname
